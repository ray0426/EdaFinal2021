...
Graph genLUT(Graph graph) {
  ...
  // 2x2
  if ((graph.maxrow - graph.minrow) == 1 && (graph.maxcol - graph.mincol) == 1)
...
Graph genPOST(Graph graph, int Case) {
  ...
  case 1:
    for (i = mincol; i < maxcol; i++)
      graph.edges.push_back(Edge(minrow, minrow, i, i + 1));
    break;
  case 2:
    for (i = minrow; i < maxrow; i++)
      graph.edges.push_back(Edge(i, i + 1, mincol, mincol));
    break;
  case 3:
    //            cout << minrow << " " << maxrow << " " <<
    //                mincol << " " << maxcol << endl;
    graph.edges.push_back(Edge(minrow, maxrow, mincol, mincol));
    graph.edges.push_back(Edge(minrow, maxrow, maxcol, maxcol));
    graph.edges.push_back(Edge(minrow, minrow, mincol, maxcol));
    break;
    //        case 4:
  default:
...
Graph CompactAndExpand(Graph graph, int bd) {
  ...
  case 0:
    ...
    for (j = graph.mincol; j <= graph.maxcol; j++) {
      if (graph1.pins[graph.minrow][j]) {
        graph.edges.push_back(Edge(graph.minrow, graph.minrow + 1, j, j));
      }
    }
    break;
  case 1:
    ...
    for (j = graph.mincol; j <= graph.maxcol; j++) {
      if (graph1.pins[graph.maxrow][j]) {
        graph.edges.push_back(Edge(graph.maxrow - 1, graph.maxrow, j, j));
      }
    }
    break;
  case 2:
    ...
    for (i = graph.minrow; i <= graph.maxrow; i++) {
      if (graph1.pins[i][graph.mincol]) {
        graph.edges.push_back(Edge(i, i, graph.mincol, graph.mincol + 1));
      }
    }
    break;
  case 3:
    ...
    for (i = graph.minrow; i <= graph.maxrow; i++) {
      if (graph1.pins[i][graph.maxcol]) {
        graph.edges.push_back(Edge(i, i, graph.maxcol - 1, graph.maxcol));
      }
    }
    break;
  default:
...
void print(Graph &graph) {
  ...
  for (j = 0; j < graph.pins[0].size() * 2 - 1; j++)
    cout << "-";
  cout << endl;
